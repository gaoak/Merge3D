...
static double neawallRegion(double x, double y, double z) {
    ...
    p[0] = x; p[1] = y; p[2] = z;
    transform(p, -AoA);
    double radius = rBoundaryLayer*0.7;
    double radiuslow = 0.5*rBoundaryLayer + 0.5*Thickness;
    double endz = spanlength + rBoundaryLayer;
    double res = radius*radius - (x*x + y*y);
    if(res>=0 && p[1]>-radiuslow && z <= endz && z>= -endz) return 1.;
    double xend = farWakeRight*0.9;
    double yend = 0.34;
    if(x>=0. && x<=xend && p[1]>=-radiuslow && y<=yend && z <= endz && z>=-endz) return 1.;
    double ylow = -x*tan(AoA) - rBoundaryLayer;
    if(p[0]>=chordLen && p[0]<=xend && y>=ylow && y<=yend && z <= endz && z>=-endz) return 1.;
    return -1;
}

static double wakeRegion(double x, double y, double z) {
    double res1 = rBoundaryLayer*rBoundaryLayer - x*x - y*y;
    double endz = spanlength + rBoundaryLayer;
    if(res1>=0 && z <= endz) return 1.;
    ...
    double ybot = pl[1]*0.9 - (x-pl[0])*tan(wakeDiffuseAngle+farWakeAoA);
    if(x>=-rBoundaryLayer*0.5 && x<=farWakeRight*0.98 && y>=ybot && y<=ytop && z <= endz && z>=-endz) return 1.;
    else return -1;
}
